...
public class FileSearch {
     ...
     * @param fileName      検索対象のファイル名 ファイル名にはワイルドカード文字として*を指定可能
    ...
    /**
     * 指定したディレクトリ[directoryPath]から、正規表現として指定された 検索対象のファイル[fileNamePattern]を再帰的に検索し、
     ...
     * @param type            該当するファイルオブジェクトは[type]により、 以下の指定が可能
     *                        TYPE_FILE_OR_DIR・・・ファイル及びディレクトリ TYPE_FILE・・・ファイル
     *                        TYPE_DIR・・・ディレクトリ
     * @param isRecursive     再帰的に検索する場合はtrue
     * @param period          検索対象として、ファイルの更新日付が指定日数経過 しているかどうかを設定可能 0の場合は対象外
